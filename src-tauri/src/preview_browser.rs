...
@keyframes __horma_pulse{from{opacity:.42;transform:scale(.78)}to{opacity:.95;transform:scale(1.08)}}@keyframes __horma_ripple{from{opacity:.95;transform:scale(.2)}to{opacity:0;transform:scale(4.2)}}@keyframes __horma_trail{from{opacity:.9;transform:scale(1)}to{opacity:0;transform:scale(.08)}}@media(prefers-reduced-motion:reduce){#__horma_browser_cursor[data-active='true']:after{animation:none}.__horma_ripple,.__horma_trail{animation-duration:.01ms!important}}`;(document.head||document.documentElement).appendChild(style);cursor=document.createElement('div');cursor.id='__horma_browser_cursor';cursor.setAttribute('aria-hidden','true');const core=document.createElement('span');core.id='__horma_browser_core';const label=document.createElement('span');label.id='__horma_browser_label';label.textContent='AI';cursor.append(core,label);badge=document.createElement('div');badge.id='__horma_browser_badge';badge.setAttribute('aria-hidden','true');badge.textContent='AI cursor · Preview Browser only';(document.body||document.documentElement).append(cursor,badge)};
  ...
  const observe=()=>{
    refs.clear();
    document.querySelectorAll('[data-horma-ai-ref]').forEach(el=>el.removeAttribute('data-horma-ai-ref'));
    const elements=[],interactive=[],scrollables=[],positions=new Map(),inspected=new Set();
    const MAX_SCAN=320,MAX_ANCESTORS=480;
    const addAncestors=seed=>{
      for(let n=seed;n&&inspected.size<MAX_ANCESTORS;n=n.parentElement){
        if(inspected.has(n))break;
        inspected.add(n);
        const p=scrollInfo(n);
        if(p&&!positions.has(n)){positions.set(n,p);scrollables.push(n)}
      }
    };
    addAncestors(document.elementFromPoint(point.x,point.y));
    let scanned=0;
    for(const el of document.querySelectorAll(QUERY)){
      if(scanned>=MAX_SCAN||interactive.length>=80)break;
      scanned++;
    ...
    for(const el of [...scrollables,...interactive]){
      if(elements.length>=80)break;
      if(emitted.has(el)||!visible(el))continue;
      emitted.add(el);
      const ref='p'+(elements.length+1),r=el.getBoundingClientRect(),input=el,p=positions.get(el);
      refs.set(ref,el);
      el.setAttribute('data-horma-ai-ref',ref);
      const safeScrollName=clip(el.getAttribute('aria-label')||el.getAttribute('title')||el.id);
      const item={
        ref,
        tag:el.tagName.toLowerCase(),
        role:clip(el.getAttribute('role')||el.tagName.toLowerCase(),48),
        name:p?(safeScrollName||'Scrollable region'):clip(el.getAttribute('aria-label')||el.getAttribute('title')||el.getAttribute('alt')||input.placeholder||el.innerText||el.textContent||input.name||el.id),
        selector:cssPath(el),
        rect:{x:Math.round(r.x),y:Math.round(r.y),width:Math.round(r.width),height:Math.round(r.height)},
        disabled:Boolean(input.disabled)
      };
      if(p){item.scrollable=true;item.scroll=p}
      if(typeof input.checked==='boolean')item.checked=input.checked;
      if('value'in input&&input.type!=='password'&&clip(input.value))item.value=clip(input.value,120);
      elements.push(item);
    }
    status(`Observed ${elements.length} target${elements.length===1?'':'s'}`);
    return{
      scope:'active-preview-tab-only',
      tabKind:'browser',
      title:clip(document.title,200),
      url:location.href,
      viewport:{width:innerWidth,height:innerHeight,scrollX:Math.round(scrollX),scrollY:Math.round(scrollY),devicePixelRatio},
      cursor:{x:Math.round(point.x),y:Math.round(point.y)},
      elements,
      hint:'Use element refs with computer_actions. Scrollable regions include scroll x/y/maxX/maxY; use their ref for nested panes. viewport.scrollY is page-only. Coordinates are relative to this Browser preview viewport.'
    }
  };
