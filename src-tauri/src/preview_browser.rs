...
(() => {
  ...
  const chooseScroll=(list,dx,dy,lock)=>{if(!list.length)return null;if(lock)return list[0];return list.find(c=>canMove(c.position,dx,dy))||list[0]};
  const refs = new Map();
