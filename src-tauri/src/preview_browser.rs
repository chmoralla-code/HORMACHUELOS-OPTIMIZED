...
(() => {
  ...
  const scrollable=el=>{if(!el||el===document.body||el===document.documentElement)return false;const p=scrollPos(el),s=getComputedStyle(el);return(p.maxX>1&&/^(auto|scroll|overlay)$/.test(s.overflowX))||(p.maxY>1&&/^(auto|scroll|overlay)$/.test(s.overflowY))};
  const canMove=(p,dx,dy)=>(dx>0&&p.x<p.maxX-1)||(dx<0&&p.x>1)||(dy>0&&p.y<p.maxY-1)||(dy<0&&p.y>1);
  const scrollCandidates=el=>{const out=[],seen=new Set();for(let n=el;n;n=n.parentElement){if(!seen.has(n)&&scrollable(n)){seen.add(n);out.push({target:n,position:scrollPos(n),kind:'nested'})}}const page=document.scrollingElement;if(page&&!seen.has(page))out.push({target:page,position:scrollPos(page),kind:'page'});return out};
...
#__horma_browser_cursor{position:fixed;z-index:2147483646;left:0;top:0;width:48px;height:48px;pointer-events:none;transform:translate3d(32px,32px,0);will-change:transform,opacity;contain:layout style paint;isolation:isolate}
#__horma_browser_cursor:before{content:'';position:absolute;inset:2px 14px 14px 2px;border-radius:6px 55% 55% 55%;background:linear-gradient(145deg,#fff,#69e9ff 38%,#775cff);clip-path:polygon(0 0,100% 68%,55% 73%,38% 100%);box-shadow:0 0 0 2px rgba(0,9,20,.86),0 0 18px 4px rgba(79,224,255,.78),0 0 34px rgba(126,92,255,.58)}
#__horma_browser_cursor:after{content:'';position:absolute;left:-11px;top:-11px;width:58px;height:58px;border:2px solid rgba(105,234,255,.56);border-radius:50%;box-shadow:inset 0 0 14px rgba(111,227,255,.18),0 0 22px rgba(103,100,255,.3);opacity:.72;transform:scale(.88)}
#__horma_browser_cursor[data-active='true']:after{animation:__horma_pulse 1s cubic-bezier(.2,.8,.2,1) infinite alternate}
#__horma_browser_core{position:absolute;left:7px;top:7px;width:8px;height:8px;border-radius:50%;background:#fff;box-shadow:0 0 0 2px rgba(83,224,255,.55),0 0 14px #fff}
#__horma_browser_label{position:absolute;left:30px;top:30px;padding:3px 6px;border:1px solid rgba(123,230,255,.72);border-radius:999px;background:rgba(4,12,24,.94);color:#effdff;font:800 10px/1 ui-monospace,Consolas,monospace;letter-spacing:.08em;box-shadow:0 4px 14px rgba(0,0,0,.42),0 0 12px rgba(91,220,255,.34)}
#__horma_browser_badge{position:fixed;z-index:2147483647;left:18px;bottom:18px;max-width:min(360px,calc(100vw - 36px));pointer-events:none;padding:9px 13px;border:1px solid rgba(107,222,255,.46);border-radius:999px;background:rgba(5,13,22,.94);color:#e9fcff;font:700 12px/1.25 ui-monospace,Consolas,monospace;letter-spacing:.05em;box-shadow:0 10px 28px rgba(0,0,0,.32)}
.__horma_ripple{position:fixed;z-index:2147483645;width:18px;height:18px;margin:-9px;border:3px solid #79efff;border-radius:50%;pointer-events:none;will-change:transform,opacity;box-shadow:0 0 18px rgba(105,103,255,.7);animation:__horma_ripple .54s cubic-bezier(.2,.8,.2,1) forwards}.__horma_trail{position:fixed;z-index:2147483644;width:12px;height:12px;margin:-6px;border-radius:50%;pointer-events:none;will-change:transform,opacity;background:#8bf0ff;box-shadow:0 0 15px #6d8cff;animation:__horma_trail .46s ease-out forwards}
@keyframes __horma_pulse{from{opacity:.42;transform:scale(.78)}to{opacity:.95;transform:scale(1.08)}}@keyframes __horma_ripple{from{opacity:.95;transform:scale(.2)}to{opacity:0;transform:scale(4.2)}}@keyframes __horma_trail{from{opacity:.9;transform:scale(1)}to{opacity:0;transform:scale(.08)}}@media(prefers-reduced-motion:reduce){#__horma_browser_cursor[data-active='true']:after{animation:none}.__horma_ripple,.__horma_trail{animation-duration:.01ms!important}}`;(document.head||document.documentElement).appendChild(style);cursor=document.createElement('div');cursor.id='__horma_browser_cursor';cursor.setAttribute('aria-hidden','true');const core=document.createElement('span');core.id='__horma_browser_core';const label=document.createElement('span');label.id='__horma_browser_label';label.textContent='AI';cursor.append(core,label);badge=document.createElement('div');badge.id='__horma_browser_badge';badge.setAttribute('aria-hidden','true');badge.textContent='AI cursor · Preview Browser only';(document.body||document.documentElement).append(cursor,badge)};
  ...
  const observe=()=>{refs.clear();document.querySelectorAll('[data-horma-ai-ref]').forEach(el=>el.removeAttribute('data-horma-ai-ref'));const elements=[],interactive=Array.from(document.querySelectorAll(QUERY)),candidates=new Set(interactive),addAncestors=seed=>{for(let n=seed;n;n=n.parentElement)if(scrollable(n))candidates.add(n)};addAncestors(document.elementFromPoint(point.x,point.y));interactive.forEach(addAncestors);for(const el of candidates){if(elements.length>=80||!visible(el))continue;const ref='p'+(elements.length+1),r=el.getBoundingClientRect(),input=el;refs.set(ref,el);el.setAttribute('data-horma-ai-ref',ref);const item={ref,tag:el.tagName.toLowerCase(),role:clip(el.getAttribute('role')||el.tagName.toLowerCase(),48),name:clip(el.getAttribute('aria-label')||el.getAttribute('title')||el.getAttribute('alt')||input.placeholder||el.innerText||el.textContent||input.name||el.id),selector:cssPath(el),rect:{x:Math.round(r.x),y:Math.round(r.y),width:Math.round(r.width),height:Math.round(r.height)},disabled:Boolean(input.disabled)};if(scrollable(el)){item.scrollable=true;item.scroll=scrollPos(el);if(!item.name)item.name='Scrollable region'}if(typeof input.checked==='boolean')item.checked=input.checked;if('value'in input&&input.type!=='password'&&clip(input.value))item.value=clip(input.value,120);elements.push(item)}const content=[],seen=new Set();let total=0;for(const el of document.querySelectorAll("h1,h2,h3,h4,h5,h6,p,li,th,td,[role='heading'],[role='row'],[role='cell']")){if(content.length>=40||total>=6000||!visible(el))continue;const text=clip(el.innerText||el.textContent,300);if(!text||seen.has(text))continue;seen.add(text);total+=text.length;const r=el.getBoundingClientRect();content.push({tag:el.tagName.toLowerCase(),text,rect:{x:Math.round(r.x),y:Math.round(r.y),width:Math.round(r.width),height:Math.round(r.height)}})}status(`Observed ${elements.length} target${elements.length===1?'':'s'}`);return{scope:'active-preview-tab-only',tabKind:'browser',title:clip(document.title,200),url:location.href,viewport:{width:innerWidth,height:innerHeight,scrollX:Math.round(scrollX),scrollY:Math.round(scrollY),devicePixelRatio},cursor:{x:Math.round(point.x),y:Math.round(point.y)},elements,content,hint:'Use element refs with computer_actions. Scrollable regions include scroll x/y/maxX/maxY; use their ref for nested panes. viewport.scrollY is page-only. Coordinates are relative to this Browser preview viewport.'}};
  const action=async(a,own)=>{const duration=clamp(a.duration_ms??170,0,900);if(a.type==='wait'){await sleep(clamp(a.duration_ms??250,0,10000),own);return{}}if(a.type==='scroll'){const explicit=Boolean(a.ref||a.selector);let t;if(explicit||(Number.isFinite(Number(a.x))&&Number.isFinite(Number(a.y))))t=resolve(a);else t={el:document.elementFromPoint(point.x,point.y)||document.scrollingElement,point};await move(t.point,own,duration);const dx=clamp(a.delta_x??0,-4000,4000),dy=clamp(a.delta_y??520,-4000,4000),candidate=chooseScroll(scrollCandidates(t.el),dx,dy);if(!candidate)throw Error('No scrollable region is available in the active Preview tab.');const before=scrollPos(candidate.target),wheelTarget=t.el||document.elementFromPoint(t.point.x,t.point.y);wheelTarget?.dispatchEvent(new WheelEvent('wheel',{bubbles:true,cancelable:true,clientX:t.point.x,clientY:t.point.y,deltaX:dx,deltaY:dy}));candidate.target.scrollBy({left:dx,top:dy,behavior:'auto'});await sleep(90,own);const after=scrollPos(candidate.target),moved=Math.abs(after.x-before.x)>1||Math.abs(after.y-before.y)>1;return{target:candidate.kind,selector:candidate.kind==='page'?null:cssPath(candidate.target),requested:{x:dx,y:dy},before,after,applied:{x:after.x-before.x,y:after.y-before.y},moved,boundary:!moved}}if(a.type==='drag'){const s=resolve(a),e=resolve(a,true);if(!s.el)throw Error('Drag start target was not found.');await move(s.point,own,duration);pointer(s.el,'pointerdown',s.point);s.el.dispatchEvent(new DragEvent('dragstart',{bubbles:true,cancelable:true}));await move(e.point,own,Math.max(220,duration));const out=e.el||document.elementFromPoint(e.point.x,e.point.y)||s.el;pointer(out,'pointermove',e.point);out.dispatchEvent(new DragEvent('dragover',{bubbles:true,cancelable:true,clientX:e.point.x,clientY:e.point.y}));out.dispatchEvent(new DragEvent('drop',{bubbles:true,cancelable:true,clientX:e.point.x,clientY:e.point.y}));pointer(out,'pointerup',e.point);s.el.dispatchEvent(new DragEvent('dragend',{bubbles:true}));return}const t=resolve(a);await move(t.point,own,duration);if(!t.el)return;if(a.type==='move'||a.type==='hover'){['pointerover','pointerenter','pointermove'].forEach(v=>pointer(t.el,v,t.point));return}if(a.type==='click'){const button=a.button==='right'?2:a.button==='middle'?1:0;pointer(t.el,'pointerover',t.point,button);pointer(t.el,'pointerdown',t.point,button);t.el.focus?.({preventScroll:true});pointer(t.el,'pointerup',t.point,button);if(button===0){const count=a.clicks===2?2:1;for(let i=0;i<count;i++)t.el.click?.();if(count===2)t.el.dispatchEvent(new MouseEvent('dblclick',{bubbles:true,cancelable:true,clientX:t.point.x,clientY:t.point.y}))}else if(button===2)t.el.dispatchEvent(new MouseEvent('contextmenu',{bubbles:true,cancelable:true,clientX:t.point.x,clientY:t.point.y,button:2}));const r=document.createElement('i');r.className='__horma_ripple';r.style.left=t.point.x+'px';r.style.top=t.point.y+'px';(document.body||document.documentElement).appendChild(r);setTimeout(()=>r.remove(),520);return}if(a.type==='type'){const el=editable(t.el)?t.el:document.activeElement;if(!editable(el))throw Error('Type target is not editable.');typeText(el,String(a.text??''),Boolean(a.clear));return}if(a.type==='key'){t.el.focus?.({preventScroll:true});keyPress(t.el,String(a.keys||''))}};
  window.__hormaPreviewComputerUse={stop(){generation++;status('Stopped');return{ok:true,scope:'active-preview-tab-only'}},observe,async actions(args){const own=++generation;if(!refs.size)observe();const list=Array.isArray(args?.actions)?args.actions:[],results=[];for(let i=0;i<list.length;i++){if(own!==generation)throw Error('Preview Computer Use stopped.');status(`${i+1}/${list.length} · ${list[i].type}`,true);const detail=await action(list[i],own)||{};results.push({index:i,type:list[i].type,ok:true,...detail})}status(`Complete · ${list.length} action${list.length===1?'':'s'}`);return{ok:true,scope:'active-preview-tab-only',completed:list.length,results,cursor:{x:Math.round(point.x),y:Math.round(point.y)}}},handle(op,args){if(op==='observe')return Promise.resolve(observe());if(op==='actions')return this.actions(args);if(op==='stop')return Promise.resolve(this.stop());return Promise.reject(Error('Unsupported Preview Computer Use operation.'))}};
})();
"###;
