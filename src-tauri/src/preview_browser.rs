...
(() => {
  ...
  let targetFx=null,viewportFx=null,labelFx=null,styleFx=null,settleTimer=null,transients=[],currentTimer=null,currentReject=null,currentAnimation=null;
  const reduced=()=>!!matchMedia('(prefers-reduced-motion: reduce)').matches;
  const ensureFx=()=>{if(cursor&&cursor.isConnected&&targetFx?.isConnected&&viewportFx?.isConnected)return;const style=document.createElement('style');style.dataset.hormaComputerUse='true';style.textContent=`
...
#__horma_browser_viewport{position:fixed!important;z-index:2147483642!important;inset:0!important;pointer-events:none!important;border-radius:0;padding:56px;background:linear-gradient(to right,#000 0,rgba(255,255,255,.38) 8px,transparent 72px),linear-gradient(to left,#000 0,rgba(255,255,255,.38) 8px,transparent 72px),linear-gradient(to bottom,#000 0,rgba(255,255,255,.38) 8px,transparent 72px),linear-gradient(to top,#000 0,rgba(255,255,255,.38) 8px,transparent 72px);box-shadow:inset 0 0 0 1px #000;animation:__horma-ai-frame-shade 5.2s ease-in-out infinite;-webkit-mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);-webkit-mask-composite:xor;mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);mask-composite:exclude}
#__horma_browser_viewport:after{content:'';position:absolute;inset:0;border-radius:inherit;padding:56px;pointer-events:none;box-shadow:inset 0 0 0 2px #fff,inset 0 0 14px 2px rgba(255,255,255,.42),inset 0 0 36px 8px rgba(255,255,255,.12);animation:__horma-ai-frame-glow 5.2s ease-in-out infinite;-webkit-mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);-webkit-mask-composite:xor;mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);mask-composite:exclude}
...
#__horma_browser_cursor:before{content:'';position:absolute;left:-28px;top:0;width:48px;height:128px;background:url(__HORMA_HAND_URI__) no-repeat 0 0/48px 128px;filter:drop-shadow(0 0 1px #000) drop-shadow(0 0 2px #000) drop-shadow(0 2px 4px rgba(0,0,0,.72));transform-origin:28px 0;transition:transform .12s cubic-bezier(.16,1,.3,1),opacity .12s ease}
#__horma_browser_cursor:after{content:'';position:absolute;left:-10px;top:-10px;width:20px;height:20px;border:1px solid rgba(255,176,64,.7);border-radius:50%;opacity:.18;transform:scale(.72);transition:transform .16s cubic-bezier(.16,1,.3,1),opacity .16s ease}
#__horma_browser_cursor[data-gesture='approach']:after{opacity:.8;transform:scale(1)}
#__horma_browser_cursor[data-gesture='hover']:before{transform:scale(1.04)}
#__horma_browser_cursor[data-gesture='hover']:after,#__horma_browser_cursor[data-gesture='press']:after,#__horma_browser_cursor[data-gesture='click']:after{opacity:1;transform:scale(1.22);border-color:#ff7a18;box-shadow:0 0 16px #ff8a1a}
#__horma_browser_cursor[data-gesture='press']:before{transform:scale(.94)}
#__horma_browser_cursor[data-gesture='click']:before{animation:__horma_browser_click .22s cubic-bezier(.16,1,.3,1)}
#__horma_browser_core{position:absolute;left:-3px;top:-3px;width:6px;height:6px;border-radius:50%;background:#fff;box-shadow:0 0 0 1px #000,0 0 8px #ff7a18;opacity:.95}
...
#__horma_browser_cursor .__horma-ai-fire{position:absolute;left:0;top:0;width:0;height:0;pointer-events:none;overflow:visible}
#__horma_browser_cursor .__horma-ai-fire i{position:absolute;left:-4px;top:-12px;width:8px;height:16px;border-radius:50% 50% 40% 40%;background:linear-gradient(#fffbe8,#ffb020 38%,#ff5a00 78%,#7a1400);box-shadow:0 0 8px #ff8a1a,0 0 18px #ff4d00;opacity:0;transform:translate3d(0,4px,0) scale(.35);transform-origin:50% 100%}
#__horma_browser_cursor .__horma-ai-fire.is-lit i:nth-child(1){animation:__horma-ai-fire-a .46s cubic-bezier(.16,1,.3,1)}
#__horma_browser_cursor .__horma-ai-fire.is-lit i:nth-child(2){animation:__horma-ai-fire-b .5s cubic-bezier(.16,1,.3,1)}
#__horma_browser_cursor .__horma-ai-fire.is-lit i:nth-child(3){animation:__horma-ai-fire-a .42s cubic-bezier(.16,1,.3,1) .04s}
#__horma_browser_cursor .__horma-ai-fire.is-lit i:nth-child(4){animation:__horma-ai-fire-b .48s cubic-bezier(.16,1,.3,1) .06s}
@keyframes __horma-ai-fire-a{0%{opacity:.95;transform:translate3d(0,2px,0) scale(.5)}60%{opacity:.8;transform:translate3d(-5px,-18px,0) scale(1)}100%{opacity:0;transform:translate3d(-7px,-30px,0) scale(.12)}}
@keyframes __horma-ai-fire-b{0%{opacity:.9;transform:translate3d(0,2px,0) scale(.42)}60%{opacity:.75;transform:translate3d(6px,-16px,0) scale(1.05)}100%{opacity:0;transform:translate3d(9px,-28px,0) scale(.1)}}
@keyframes __horma_browser_click{0%{transform:scale(.86)}55%{transform:scale(1.08)}100%{transform:scale(1)}}@media(prefers-reduced-motion:reduce){#__horma_browser_cursor:before,#__horma_browser_cursor:after,#__horma_browser_target,#__horma_browser_viewport,#__horma_browser_viewport:after,#__horma_browser_label,.__horma-ai-fire i{animation:none!important;transition-duration:.01ms!important}.__horma_browser_trail,.__horma_browser_wave,.__horma-ai-fire{display:none!important}}`;styleFx=style;(document.head||document.documentElement).appendChild(style);cursor=document.createElement('div');cursor.id='__horma_browser_cursor';cursor.setAttribute('aria-hidden','true');const core=document.createElement('span');core.id='__horma_browser_core';labelFx=document.createElement('span');labelFx.id='__horma_browser_label';labelFx.textContent='AI';const fire=document.createElement('span');fire.className='__horma-ai-fire';fire.setAttribute('aria-hidden','true');for(let i=0;i<4;i++)fire.appendChild(document.createElement('i'));cursor.append(core,labelFx,fire);targetFx=document.createElement('div');targetFx.id='__horma_browser_target';targetFx.setAttribute('aria-hidden','true');const targetLabel=document.createElement('span');targetLabel.id='__horma_browser_target_label';targetLabel.textContent='TARGET';targetFx.append(targetLabel);targetFx._label=targetLabel;viewportFx=document.createElement('div');viewportFx.id='__horma_browser_viewport';viewportFx.setAttribute('aria-hidden','true');badge=document.createElement('div');badge.id='__horma_browser_badge';badge.setAttribute('aria-hidden','true');badge.textContent='AI cursor · Preview Browser only';(document.body||document.documentElement).append(viewportFx,targetFx,cursor,badge);installTakeover()};
  ...
  const status=(text,active=false)=>{ensureFx();badge.textContent='AI cursor · '+text;badge.dataset.active=String(active);cursor.dataset.visible='true';cursor.dataset.busy=String(active||cursor.dataset.gesture!=='idle')};
  const gesture=(name,label=name.toUpperCase())=>{ensureFx();cursor.dataset.visible='true';cursor.dataset.busy=String(name!=='idle');cursor.dataset.gesture=name;labelFx.textContent='AI · '+label;if(['hover','press','click','type','key'].includes(name)){const f=cursor.querySelector?.('.__horma-ai-fire');if(f){f.classList.remove('is-lit');void f.offsetWidth;f.classList.add('is-lit')}}};
  ...
  const move=async(p,own,ms)=>{if(own!==generation)throw Error('Preview Computer Use stopped.');ensureFx();const end={x:clamp(p.x,0,innerWidth-1),y:clamp(p.y,0,innerHeight-1)},dist=Math.hypot(end.x-point.x,end.y-point.y),requested=ms==null?45+dist*.22:Number(ms),duration=reduced()||dist<2?0:(requested===0?0:clamp(requested,45,120));gesture('approach','TARGET');trail(point,end);if(duration&&cursor.animate){const start={...point},a=cursor.animate([{transform:`translate3d(${start.x}px,${start.y}px,0)`},{transform:`translate3d(${(start.x+end.x)/2}px,${Math.min(start.y,end.y)-Math.min(18,dist*.06)}px,0)`},{transform:`translate3d(${end.x}px,${end.y}px,0)`}],{duration,easing:'cubic-bezier(.16,1,.3,1)',fill:'forwards'});currentAnimation=a;try{await a.finished}catch{if(own!==generation)throw Error('Preview Computer Use stopped.')}finally{a.cancel();if(currentAnimation===a)currentAnimation=null}}if(own!==generation)throw Error('Preview Computer Use stopped.');place(end)};
  ...
  const mouse=(el,type,p,button=0,extra={})=>el.dispatchEvent(new MouseEvent(type,{bubbles:true,cancelable:true,composed:true,view:window,clientX:p.x,clientY:p.y,screenX:p.x,screenY:p.y,button,buttons:type==='mousedown'?1<<button:0,detail:type==='click'||type==='dblclick'?1:0,...extra}));
  ...
  const clickable=el=>{if(!el)return false;if(el.getAttribute?.('onclick')||el.getAttribute?.('href')||el.getAttribute?.('data-href')||el.getAttribute?.('data-url'))return true;if(el.hasAttribute?.('tabindex')&&el.tabIndex>=0)return true;try{return getComputedStyle(el).cursor==='pointer'}catch{return false}};
  const activate=(seed,p)=>{const hit=document.elementFromPoint(p.x,p.y)||seed;try{const direct=hit.closest?.("a[href],button,summary,input[type='button'],input[type='submit'],input[type='reset'],input[type='image'],[role='button'],[role='link'],[role='menuitem'],[role='tab'],[role='option']");if(direct&&visible(direct))return direct}catch{}const host=(()=>{try{return hit.closest?.("tr,[role='row'],[role='listitem'],[role='gridcell'],li,article,[data-href]")||seed}catch{return seed}})();try{for(const n of host.querySelectorAll?.('a[href],[role=link]')||[]){if(visible(n))return n}}catch{}let node=hit;for(let i=0;i<10&&node;i++,node=node.parentElement){if(node===document.body||node===document.documentElement)break;if(clickable(node))return node}return hit};
