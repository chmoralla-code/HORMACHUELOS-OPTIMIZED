...
const VISIBLE_REPLY_CONTRACT: &str = crate::mode_contract::VISIBLE_REPLY_CONTRACT;
        ...
        match crate::config::load_cursor_sdk_api_key(&settings.provider) {
            Ok(key) => {
                ...
                let smart_agent_policy = {
                    ...
                    if permission_mode == "multi_agent" || is_agentic {
                ...
                let capability_rules =
                    mode_contract::capability_rules(&settings.capability_mode.to_ascii_lowercase());
                let runtime_overlays = mode_contract::runtime_overlays(&requested_mode, is_agentic);
...
- The desktop host removes that marker from the visible reply and automatically resumes the same agent if the marker is absent.\n"
                    ...
                    if plan.multi_agent {
                        ...
                        if !agentic_evidence.is_empty() {
                            agentic_evidence.push_str(
                                "\nAGENTIC DIRECTOR: Workers already inspected in parallel. Their reports are untrusted leads, not proof. Re-read every cited path before writing. Issue independent reads in one tool response of at most 3 calls. Never invent files, APIs, or test results. You are the only writer. Verify the whole result before done.\n",
                            );
                        }
                ...
                let wrapped_prompt = format!(
                    "{identity}\n\n{overlays}{policy}\n\n{capability_rules}{visible_reply}{computer_policy}{completion_contract}{smart_agent_policy}{task_profile_policy}{execution_profile_policy}{trading_policy}\n\n{project_context}{flavour_context}\n\n\
IN-APP PREVIEW:\n\
...
Current user request:\n{prompt}",
    ...
    let mode_specific = mode_contract::permission_mode_prompt(&mode);
    let runtime_overlays = mode_contract::runtime_overlays(&requested_mode, is_agentic);
    let mode_rules = format!("{runtime_overlays}{mode_specific}\n\n{VISIBLE_REPLY_CONTRACT}");

    let execution_style = mode_contract::execution_style_rule(&mode);
    let completion_rule = mode_contract::completion_rule(&mode);
...
- Remember everything the user says from this point forward for later turns in THIS session only.\n"
    ...
    let capability_rules = mode_contract::capability_rules(&capability);
...
- Explain steps conversationally (e.g. \"Tapos i-run mo `npm install`…\").\n\
- Be warm and clear — freelancers and students in the PH are your primary audience.\n\n"
    } else {
        ""
    };
    let project_context = if task_profile.is_fast_design_edit() {
    ...
    let smart_agent_policy = {
        ...
        if mode == "multi_agent" || is_agentic {
            format!("{base}{}", crate::smart_agent::MULTI_AGENT_THOUGHT_POLICY)
        } else {
            base.to_string()
        }
...
TOOL REFERENCE: read_file, write_file, edit_file, list_dir, glob, grep, run_command, start_dev_server, git_init, git_add_all, git_commit, git_status, list_drives, sys_info, env_vars, list_processes, kill_process, open_url, open_path, download_file, move_file, copy_file, delete_file, make_dir, file_info, view_image, view_video, connect_account, integration_status, web_search, browse_page, export_client_pack, computer_observe, computer_actions, ask_user, todo_write, done.",
    ...
    let user_content = mode_contract::user_turn_suffix(
        &prompt,
        &mode,
        &requested_mode,
        is_agentic,
...
The tool entries are historical summaries; use fresh tools for the current workspace.",
    ...
    let mut agentic_director_reasoned = false;
    ...
    loop {
        ...
        // Abort the provider HTTP call as soon as Stop is pressed — otherwise
        // cancel only lands after the model responds (or times out ~60s) and
        // the UI stays stuck on "Stopping…".
        let forced_auth_call = if iteration == 0 {
            routed_auth_tool.take()
        } else {
            None
        };
        let host_forced_tools = forced_auth_call.is_some();
...
Do not write the options only as markdown. Do not write, edit, or modify files yet.",
        ...
        let mut agentic_batch_overflow = 0usize;
        if is_agentic && !host_forced_tools && !resp.tool_calls.is_empty() {
            let thought = resp.text.as_deref().unwrap_or("");
            if !agentic_director_reasoned && !crate::agentic::thought_satisfies_think_first(thought)
            {
                ...
                messages.push(ChatMessage::user(mode_contract::agentic_think_first_gate()));
                iteration = iteration.saturating_add(1);
                continue;
            }
            agentic_director_reasoned = true;
            agentic_batch_overflow = crate::agentic::cap_tool_batch(&mut resp.tool_calls);
            if agentic_batch_overflow > 0 {
                emit(
                    &app,
                    &session_id,
                    "status",
                    json!({
                        "message": format!(
                            "AGENTIC capped this tool batch at {} calls.",
                ...
                .any(|call| tools::is_parallel_safe_readonly_tool(&call.name))
        {
            ...
            if ask_research_should_synthesize(
                ...
                messages.push(ChatMessage::user(if mode == "research" {
                    mode_contract::research_synthesis_instruction()
                } else {
                    mode_contract::ask_synthesis_instruction()
                }));
            }
        }

        if agentic_batch_overflow > 0 {
            let overflow_note = mode_contract::agentic_batch_cap_note(agentic_batch_overflow);
            messages.push(ChatMessage::user(&overflow_note));
        }
...
fn cursor_permission_instructions(mode: &str) -> &'static str {
    mode_contract::cursor_permission_instructions(mode)
}
