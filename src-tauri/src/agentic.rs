...
pub enum AgenticPhase { Ask, Plan, Research, MultiAgent, Build }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum AgenticPhaseState { Pending, Active, Completed, Skipped, Failed, Cancelled }
...
impl AgenticWorkerResult {
    fn new(spec: &AgenticWorkerSpec, status: &str) -> Self {
        Self {
            id: spec.id.clone(), name: spec.name.clone(), role: spec.role.clone(),
            assignment: spec.assignment.clone(), status: status.into(),
            tool_count: 0, total_tokens: 0, result_summary: String::new(), error: None,
        }
    }
}
...
impl AgenticPlan {
    pub fn classify(request: &str) -> Self {
        let input = request.to_ascii_lowercase().split_whitespace().collect::<Vec<_>>().join(" ");
        let plan_only = has_any(&input, &[
            "do not implement", "don't implement", "dont implement", "plan only",
            "no implementation", "without changing", "without changes", "research only", "read-only",
        ]);
        let mutation = !plan_only && has_any(&format!(" {input} "), &[
            " implement ", " fix ", " add ", " build ", " create ", " change ",
            " update ", " improve ", " refactor ", " remove ", " apply ", " tweak ",
        ]);
        let investigation = has_any(&input, &[
            "audit", "research", "investigate", "analyze", "analyse",
            "review architecture", "security review", "assess",
        ]);
        let explicit_plan = has_any(&input, &["create a plan", "make a plan", "plan for", "plan this"]);
        let simple = !mutation && !investigation && !explicit_plan
            && (input.ends_with('?') || has_any(&input, &[
                "what does", "what is", "how does", "why does", "where is", "explain ",
            ]));
        ...
        let research = !simple && (mutation || investigation || multi_agent
            || has_any(&input, &["project", "workspace", "codebase", "component", "file"]));
        let workers = if multi_agent {
            domains.into_iter().take(MAX_AGENTIC_WORKERS).enumerate().map(
                |(index, (role, assignment))| AgenticWorkerSpec {
        ...
        Self { plan, research, multi_agent, build: mutation, workers }
    }

    pub const fn effective_mode(&self) -> &'static str {
        if self.build { "build" } else if self.research { "research" }
        else if self.plan { "plan" } else { "ask" }
    }
...
struct RunEvent { kind: String, session_id: String, payload: Value }

fn emit(app: &AppHandle, session_id: &str, kind: &str, payload: Value) {
    let _ = app.emit("agent", RunEvent { kind: kind.into(), session_id: session_id.into(), payload });
}

pub fn emit_plan(app: &AppHandle, session_id: &str, plan: &AgenticPlan) {
    let phases = [
        AgenticPhase::Ask, AgenticPhase::Plan, AgenticPhase::Research,
        AgenticPhase::MultiAgent, AgenticPhase::Build,
    ].into_iter().map(|phase| json!({
        "phase": phase.wire(),
        "state": if phase == AgenticPhase::Ask { "active" }
            else if plan.enabled(phase) { "pending" } else { "skipped" },
    })).collect::<Vec<_>>();
    emit(app, session_id, "agentic_plan", json!({
        "run_id": session_id, "phases": phases, "max_workers": MAX_AGENTIC_WORKERS,
    }));
    emit_agent(app, session_id, &AgenticWorkerResult {
        id: "director".into(), name: "Director".into(),
        ...
        status: "running".into(), tool_count: 0, total_tokens: 0,
        result_summary: String::new(), error: None,
    });
}

pub fn emit_phase(
    app: &AppHandle, session_id: &str, phase: AgenticPhase,
    state: AgenticPhaseState, detail: impl AsRef<str>,
) {
    emit(app, session_id, "agentic_phase", json!({
...
pub fn emit_agent(app: &AppHandle, session_id: &str, worker: &AgenticWorkerResult) {
    emit(app, session_id, "agentic_agent", json!({
        "run_id": session_id,
        "agent": {
...
pub fn worker_tool_allowed(agent_id: &str, raw_name: &str) -> bool {
    if agent_id.trim().is_empty() || agent_id == "director" { return false; }
    matches!(tools::normalize_tool_name(raw_name).as_str(),
        "read_file" | "list_dir" | "glob" | "grep" | "git_status" | "file_info"
        | "view_image" | "view_video" | "web_search" | "browse_page")
}

pub async fn run_native_workers(
    app: Arc<AppHandle>, session_id: &str, root: &Path, request: &str,
    config: NativeWorkerConfig, run: Arc<SessionRun>, plan: &AgenticPlan,
) -> Vec<AgenticWorkerResult> {
    if plan.workers.len() < 2 { return Vec::new(); }
    let mut specs = refine_specs(&config, request, &plan.workers, run.clone()).await;
    if specs.len() < 2 { specs = plan.workers.clone(); }
    specs.truncate(MAX_AGENTIC_WORKERS);
    enforce_budget(&app, session_id, &config, &mut specs);
    if specs.len() < 2 {
        emit_phase(&app, session_id, AgenticPhase::MultiAgent, AgenticPhaseState::Skipped,
    ...
    emit_phase(&app, session_id, AgenticPhase::MultiAgent, AgenticPhaseState::Active,
        format!("Running {} isolated read-only evidence workers.", specs.len()));

    let mut tasks = JoinSet::new();
    for spec in specs.clone() {
        let app = app.clone(); let sid = session_id.to_string(); let root = root.to_path_buf();
        let request = request.to_string(); let config = config.clone(); let run = run.clone();
        tasks.spawn(async move { run_worker(app, &sid, root, request, config, run, spec).await });
    }
    let mut results = Vec::new();
    while let Some(joined) = tasks.join_next().await {
        if let Ok(result) = joined { results.push(result); }
    }
    ...
    let retries = results.iter().enumerate().filter_map(|(index, worker)| {
        worker.error.as_deref().filter(|error| is_concurrency_error(error)).map(|_| index)
    }).collect::<Vec<_>>();
    if !retries.is_empty() && !run.cancel.load(Ordering::SeqCst) {
        ...
        for index in retries {
            let Some(spec) = specs.iter().find(|item| item.id == results[index].id).cloned() else { continue };
            results[index] = run_worker(
                app.clone(), session_id, root.to_path_buf(), request.to_string(),
                config.clone(), run.clone(), spec,
            ).await;
        }
    }
    let completed = results.iter().filter(|item| item.status == "completed").count();
    emit_phase(&app, session_id, AgenticPhase::MultiAgent,
        if completed == 0 { AgenticPhaseState::Failed } else { AgenticPhaseState::Completed },
        format!("{completed} of {} evidence workers returned conclusions.", results.len()));
    results
}

async fn run_worker(
    app: Arc<AppHandle>, session_id: &str, root: PathBuf, request: String,
    config: NativeWorkerConfig, run: Arc<SessionRun>, spec: AgenticWorkerSpec,
) -> AgenticWorkerResult {
    ...
    let provider = match llm::build_provider_with_effort(
        &config.provider, &config.api_key, config.base_url.as_deref(),
        &config.model, Some(&config.effort),
    ) {
        Ok(provider) => provider,
        Err(error) => {
            worker.status = "failed".into(); worker.error = Some(error.to_string());
            worker.result_summary = "Provider setup failed for this evidence worker.".into();
            emit_agent(&app, session_id, &worker); return worker;
        }
    };
    let schemas = tools::schemas_with(false, false).into_iter().filter(|schema| {
        schema.get("function").and_then(|v| v.get("name")).and_then(Value::as_str)
    ...
    let context = ToolRunContext {
        cancel: run.cancel.clone(), active_pid: run.active_pid.clone(),
        on_console_line: None, checkpoint: None, protect_command_changes: false,
    };
    ...
    for _ in 0..MAX_WORKER_ROUNDS {
        if run.cancel.load(Ordering::SeqCst) {
            worker.status = "cancelled".into(); break;
        }
        let response = match worker_chat(provider.as_ref(), &messages, &schemas, run.cancel.clone()).await {
            Ok(response) => response,
            Err(error) => {
                worker.status = "failed".into(); worker.error = Some(error.to_string()); break;
            }
        };
        worker.total_tokens = worker.total_tokens.saturating_add(response.usage_tokens);
        if let Some(text) = response.text.as_deref().map(str::trim).filter(|text| !text.is_empty()) {
            conclusion = integration_chat::redact_sensitive_text(text, &secrets);
        }
        if response.tool_calls.is_empty() { break; }
        messages.push(ChatMessage::assistant(
            response.text.as_deref().unwrap_or(""), Some(response.tool_calls.clone()), None,
        ));
        for mut call in response.tool_calls {
            if worker.tool_count >= MAX_WORKER_TOOLS {
                messages.push(ChatMessage::tool(&call.id, &call.name, "Tool budget reached; synthesize now."));
                continue;
            }
            call.name = tools::normalize_tool_name(&call.name);
            tools::normalize_tool_arguments(&call.name, &mut call.arguments);
            worker.tool_count += 1;
            emit(&app, session_id, "tool_call", json!({
            ...
            emit(&app, session_id, "tool_result", json!({
                "id": call.id.clone(), "name": call.name.clone(), "ok": ok,
                "content": bounded(&content, 4_000), "streamed": false,
                "run_id": session_id, "agent_id": spec.id.clone(), "phase": "multi_agent",
            }));
            messages.push(ChatMessage::tool(&call.id, &call.name, &bounded(&content, 12_000)));
            emit_agent(&app, session_id, &worker);
        }
    }
    if config.hosted && worker.total_tokens > 0 {
        let _ = crate::license::record_provider_usage(&config.provider, &config.model, worker.total_tokens);
    }
    if worker.status == "running" { worker.status = "completed".into(); }
    ...
    } else { bounded(&conclusion, 1_400) };
    emit_agent(&app, session_id, &worker);
    worker
}

async fn worker_chat(
    provider: &dyn llm::LlmProvider, messages: &[ChatMessage], schemas: &[Value],
    cancel: Arc<std::sync::atomic::AtomicBool>,
) -> Result<crate::llm::LlmResponse> {
    let mut retried = false;
    loop {
        if cancel.load(Ordering::SeqCst) { return Err(anyhow!("Worker cancelled.")); }
...
async fn refine_specs(
    config: &NativeWorkerConfig, request: &str, fallback: &[AgenticWorkerSpec], run: Arc<SessionRun>,
) -> Vec<AgenticWorkerSpec> {
    let Ok(provider) = llm::build_provider_with_effort(
        &config.provider, &config.api_key, config.base_url.as_deref(),
        &config.model, Some(&config.effort),
    ) else { return fallback.to_vec(); };
    let base = format!(
    ...
    for attempt in 0..2 {
        if run.cancel.load(Ordering::SeqCst) { break; }
        let prompt = if attempt == 0 { base.clone() } else {
            format!("{}\nReturn exactly one valid JSON object, no markdown.", base)
        };
        let messages = vec![
        ...
        if let Ok(response) = provider.chat(&messages, &[], None, None, None).await {
            if let Some(specs) = response.text.as_deref().and_then(parse_specs) { return specs; }
        }
    }
    fallback.to_vec()
}
...
fn parse_specs(text: &str) -> Option<Vec<AgenticWorkerSpec>> {
    let workers = serde_json::from_str::<Value>(text.trim()).ok()?.get("workers")?.as_array()?.clone();
    if !(2..=3).contains(&workers.len()) { return None; }
    workers.iter().enumerate().map(|(index, worker)| {
        let role = worker.get("role")?.as_str()?.trim();
        let assignment = worker.get("assignment")?.as_str()?.trim();
        if role.is_empty() || assignment.len() < 8 || assignment.chars().count() > 240 { return None; }
        Some(AgenticWorkerSpec {
            id: format!("worker-{}", index + 1), name: format!("Worker {}", index + 1),
            role: role.into(), assignment: assignment.into(),
        })
    }).collect()
}

pub fn evidence_context(workers: &[AgenticWorkerResult]) -> String {
    let body = workers.iter().filter(|worker| worker.status == "completed").map(|worker| {
        format!("- {} ({}): {}", worker.name, worker.role, bounded(&worker.result_summary, 1_400))
    }).collect::<Vec<_>>().join("\n");
...
fn enforce_budget(
    app: &AppHandle, session_id: &str, config: &NativeWorkerConfig, specs: &mut Vec<AgenticWorkerSpec>,
) {
    if !config.hosted { return; }
    let Ok(license) = crate::license::LicenseStatus::load() else { return };
    if license.token_budget == 0 { return; }
    ...
    if affordable < specs.len() {
        ...
        emit_phase(app, session_id, AgenticPhase::MultiAgent, AgenticPhaseState::Failed,
...
fn independent_domains(input: &str) -> Vec<(&'static str, &'static str)> {
    let candidates: [(&str, &str, &[&str]); 6] = [
        ("Frontend reviewer", "Inspect frontend interaction, accessibility, and layout evidence.",
            &["frontend", "ui", "ux", "layout", "css", "accessibility"]),
        ("Backend reviewer", "Inspect backend orchestration, events, persistence, and runtime safety.",
            &["backend", "rust", "tauri", "server", "api", "orchestration"]),
        ("Security reviewer", "Audit permission boundaries, redaction, and mutation safeguards.",
            &["security", "permission", "credential", "privacy", "safety"]),
        ("Test reviewer", "Inspect tests, CI, regressions, and verification gaps.",
            &["test", "tests", "testing", "playwright", "ci", "verification"]),
        ("Architecture reviewer", "Map component boundaries, ownership, and integration risks.",
            &["architecture", "architectural", "codebase"]),
        ("Performance reviewer", "Inspect rendering, concurrency, cancellation, and resource use.",
            &["performance", "fps", "speed", "latency", "memory"]),
    ];
    candidates.into_iter()
        .filter(|(_, _, words)| words.iter().any(|word| input.contains(word)))
        .map(|(role, assignment, _)| (role, assignment)).collect()
}

fn cancelled_workers(
    app: &AppHandle, session_id: &str, specs: &[AgenticWorkerSpec],
) -> Vec<AgenticWorkerResult> {
    specs.iter().map(|spec| {
        let mut worker = AgenticWorkerResult::new(spec, "cancelled");
        worker.result_summary = "Cancelled with the parent run.".into();
        emit_agent(app, session_id, &worker); worker
    }).collect()
}
...
fn is_concurrency_error(error: &str) -> bool {
    has_any(&error.to_ascii_lowercase(), &["429", "rate limit", "too many requests", "concurrent"])
}

fn bounded(value: &str, max: usize) -> String {
    let clean = value.replace('\0', "").split_whitespace().collect::<Vec<_>>().join(" ");
    if clean.chars().count() <= max { clean }
    else { format!("{}.", clean.chars().take(max.saturating_sub(1)).collect::<String>()) }
}
...
mod tests {
    ...
    fn workers_cannot_mutate_or_interact() {
        for name in [
            ...
            "connect_account", "ask_user", "computer_actions", "open_path", "done",
        ] {
            assert!(!worker_tool_allowed("worker-1", name), "{name}");
        }
        for name in ["read_file", "list_dir", "glob", "grep", "view_image", "web_search"] {
            assert!(worker_tool_allowed("worker-1", name), "{name}");
        }
    }
}
