//! Native Windows Computer Use broker.
//!
//! The agent never receives raw Win32 handles without a short-lived observation
//! token. Mutating actions require a fresh token and the Cursor bridge consumes
//! each token once, forcing observe -> one action -> observe.
...
const HELPER_FLAG: &str = "--computer-use-helper";
const SESSION_ENV: &str = "AI_FORGE_COMPUTER_SESSION";
const PAUSE_SENTINEL_ENV: &str = "AI_FORGE_COMPUTER_PAUSE_SENTINEL";
...
pub fn is_paused() -> bool {
    PAUSED.load(Ordering::SeqCst) || pause_sentinel_exists()
}
...
pub fn set_paused(paused: bool) {
    if paused {
        ...
        if let Err(error) = create_pause_sentinel() {
            log::error!("Could not publish the Computer Use pause state: {error}");
        }
    } else if let Err(error) = clear_pause_sentinel() {
        // Resume fails closed: the UI and helper must agree before input resumes.
        PAUSED.store(true, Ordering::SeqCst);
        log::error!("Could not clear the Computer Use pause state: {error}");
    } else {
...
fn pause_sentinel_path() -> PathBuf {
    ...
    base.join("AI-Forge").join("ComputerUse").join("paused")
}
...
pub fn run_helper_if_requested() -> bool {
    ...
    let response = (|| -> Result<Value> {
            ...
            std::env::var(SESSION_ENV)
                ...
                .unwrap_or(false),
            "Computer Use helper may only be called by an active AI-Forge session."
        );
...
#[cfg(windows)]
pub fn install_emergency_hotkey(app: tauri::AppHandle) {
    ...
    std::thread::spawn(move || unsafe {
        ...
        .is_err()
        {
            log::warn!("Computer Use emergency shortcut could not be registered.");
            return;
        }
...
#[cfg(not(windows))]
pub fn install_emergency_hotkey(_app: tauri::AppHandle) {
    initialize_pause_guard();
}
...
mod platform {
    ...
    fn classify_blocked(title: &str, process: &str) -> Option<&'static str> {
        ...
        if matches!(
            ...
            return Some("This application is protected from Computer Use.");
        }
    ...
    pub(super) fn list_windows_json() -> Result<Value> {
        Ok(json!({
            ...
            "safety": "Terminal, authentication, password-manager, security, ChatGPT, Codex, and AI-Forge windows are excluded."
        }))
    }
    ...
    fn animate_cursor_to(to_x: i32, to_y: i32) {
        let Ok((from_x, from_y)) = cursor_pos() else {
            crate::computer_fx::cursor_move(to_x, to_y);
            return;
        };
        let dx = to_x - from_x;
        let dy = to_y - from_y;
    ...
    pub(super) fn type_text(claims: &ObservationClaims, text: &str) -> Result<Value> {
        ...
        for (index, ch) in chars.into_iter().enumerate() {
            ...
            crate::computer_fx::type_char(screen_x, screen_y, &preview, idx, total);
        ...
        crate::computer_fx::type_done(screen_x, screen_y, text, total);
        Ok(json!({
            "acted": true,
            "action": "type_text",
            "window_id": claims.window_id,
    ...
    pub(super) fn press_key(claims: &ObservationClaims, keys: &str) -> Result<Value> {
        ...
        animate_cursor_to(screen_x, screen_y);
        crate::computer_fx::click(screen_x, screen_y, "key");
    ...
    pub(super) fn scroll(
        ...
        let (screen_x, screen_y) = point_in_window(&info, x, y)?;
        animate_cursor_to(screen_x, screen_y);
        crate::computer_fx::scroll(screen_x, screen_y, delta_y);
        unsafe { SetCursorPos(screen_x, screen_y)? };
        ensure_not_paused()?;
    ...
    pub(super) fn drag(
        ...
        crate::computer_fx::drag(from_x, from_y, to_x, to_y);
